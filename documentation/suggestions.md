...
- Market Data API (suggest FastAPI or NestJS)
  ...
  - GET `/stocks/{symbol}/history?period=1y` → `{ data: HistoricalData[] }`.
  - GET `/stocks/search?q=query` → array of matches.
...
- Analysis API
  ...
  - GET `/analyze/{symbol}/advanced?indicators=a,b` → `{ advanced_indicators }`.
