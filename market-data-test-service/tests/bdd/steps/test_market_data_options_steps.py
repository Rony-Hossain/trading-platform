...
scenarios(str(Path(__file__).resolve().parent.parent / "market_data_options.feature"))
...
def json_array_step(field: str, bdd_context: MutableMapping[str, object]) -> None:
    response: httpx.Response = bdd_context["response"]  # type: ignore[assignment]
    payload = response.json()
    data = payload.get(field) if isinstance(payload, Dict) else None
    assert isinstance(data, list) and data, f"{field} should be a non-empty list (got {payload})"
